...
export async function dashboardApiRoutes(app: FastifyInstance) {
  ...
  app.patch('/me/email', {
    ...
    return withUserTenant(request, reply, async (user) => {
      ...
      if (!trimmed || !trimmed.includes('@') || trimmed.length > 120) {
        return reply.code(400).send({ error: 'invalid_email' });
      }
      const db = getTenantDb();
      const existing = await db.staffUser.findFirst({
        where: { email: trimmed, NOT: { id: user.sub } },
        select: { id: true },
      });
      if (existing) {
    ...
    return withUserTenant(request, reply, async () => {
      ...
      const staff = await db.staff.findMany({
        ...
        include: {
          ...
          workingHours: true,
          timeOff: { where: { end: { gte: new Date() } }, orderBy: { start: 'asc' } },
          _count: { select: { appointments: { where: { status: { in: ['CONFIRMED', 'CONFIRMED_PAID', 'COMPLETED'] } } } } },
        },
        orderBy: { sortOrder: 'asc' },
      });
      return { staff };
  ...
  app.post('/staff', { config: { rateLimit: { max: 30, timeWindow: '1 hour' } } }, async (request, reply) => {
    return withUserTenant(request, reply, async (user) => {
      if (user.role !== 'OWNER' && user.role !== 'MANAGER') return reply.status(403).send({ error: 'Forbidden' });
      const db = getTenantDb();
      const body = request.body as {
        name: string; displayName?: string; bio?: string; specialties?: string[];
        avatarUrl?: string; isBookable?: boolean;
      };
      if (!body.name?.trim()) return reply.status(400).send({ error: 'Name is required' });
      if (body.avatarUrl && body.avatarUrl.length > 600_000) return reply.status(400).send({ error: 'Avatar too large' });
      if (body.avatarUrl && !body.avatarUrl.startsWith('data:image/') && !/^https?:\/\//.test(body.avatarUrl)) {
        return reply.status(400).send({ error: 'Invalid avatar URL' });
      }
      const count = await db.staff.count({ where: { deletedAt: null } });
  ...
  app.patch('/staff/:id', async (request, reply) => {
    return withUserTenant(request, reply, async (user) => {
      ...
      if (!existing || existing.salonId !== user.salonId) return reply.status(404).send({ error: 'Not found' });
      const body = request.body as {
        name?: string; displayName?: string; bio?: string; specialties?: string[];
        avatarUrl?: string | null; isBookable?: boolean; active?: boolean; sortOrder?: number;
      };
      if (body.avatarUrl && body.avatarUrl.length > 600_000) return reply.status(400).send({ error: 'Avatar too large' });
      if (body.avatarUrl && !body.avatarUrl.startsWith('data:image/') && !/^https?:\/\//.test(body.avatarUrl)) {
        return reply.status(400).send({ error: 'Invalid avatar URL' });
  ...
  app.delete('/staff/:id', async (request, reply) => {
    return withUserTenant(request, reply, async (user) => {
      ...
      const existing = await db.staff.findFirst({ where: { id, deletedAt: null } });
      if (!existing || existing.salonId !== user.salonId) return reply.status(404).send({ error: 'Not found' });
      await db.staff.update({ where: { id }, data: { deletedAt: new Date(), active: false } });
      return { ok: true };
    });
  });

  app.get('/staff/:id/time-off', async (request, reply) => {
    return withUserTenant(request, reply, async (user) => {
      const db = getTenantDb();
      const { id } = request.params as { id: string };
      const existing = await db.staff.findFirst({ where: { id, salonId: user.salonId, deletedAt: null } });
  ...
  app.post('/staff/:id/time-off', async (request, reply) => {
    return withUserTenant(request, reply, async (user) => {
      if (user.role !== 'OWNER' && user.role !== 'MANAGER') return reply.status(403).send({ error: 'Forbidden' });
      const db = getTenantDb();
      const { id } = request.params as { id: string };
      const existing = await db.staff.findFirst({ where: { id, salonId: user.salonId, deletedAt: null } });
      if (!existing) return reply.status(404).send({ error: 'Not found' });
      const body = request.body as { start: string; end: string; reason?: string };
      if (!body.start || !body.end) return reply.status(400).send({ error: 'start and end are required' });
      const start = new Date(body.start);
      const end = new Date(body.end);
      if (isNaN(start.getTime()) || isNaN(end.getTime())) return reply.status(400).send({ error: 'Invalid dates' });
      if (end <= start) return reply.status(400).send({ error: 'end must be after start' });
      const entry = await db.timeOff.create({
        data: { staffId: id, start, end, reason: body.reason?.trim() || null },
      });
      return { timeOff: entry };
    });
  });

  app.delete('/staff/:id/time-off/:timeOffId', async (request, reply) => {
    return withUserTenant(request, reply, async (user) => {
      if (user.role !== 'OWNER' && user.role !== 'MANAGER') return reply.status(403).send({ error: 'Forbidden' });
      const db = getTenantDb();
      const { id, timeOffId } = request.params as { id: string; timeOffId: string };
      const entry = await db.timeOff.findFirst({ where: { id: timeOffId, staffId: id, staff: { salonId: user.salonId } } });
  ...
  app.put('/staff/:id/working-hours', async (request, reply) => {
    return withUserTenant(request, reply, async (user) => {
      if (user.role !== 'OWNER' && user.role !== 'MANAGER') return reply.status(403).send({ error: 'Forbidden' });
      const db = getTenantDb();
      const { id } = request.params as { id: string };
      const existing = await db.staff.findFirst({ where: { id, salonId: user.salonId, deletedAt: null } });
      if (!existing) return reply.status(404).send({ error: 'Not found' });
      const body = request.body as { hours: Array<{ weekday: number; startTime: string; endTime: string }> };
      if (!Array.isArray(body.hours)) return reply.status(400).send({ error: 'hours must be an array' });
      ...
      const hours = await db.workingHour.findMany({ where: { staffId: id }, orderBy: { weekday: 'asc' } });
      return { hours };
    });
  });
